...
import {
  findNextSundayWorshipService,
  findPreviousSundayWorshipService,
...
export async function getPreviousSundayWorshipService(): Promise<SundaysAndSpecialDays | null> {
  const supabase = await createClient();
  const previousSunday = findPreviousSundayWorshipService();

  // Extract just the date part (YYYY-MM-DD) for querying
  // This avoids timezone/time precision issues
  const dateOnly = previousSunday.toISOString().split("T")[0];
  ...
  if (error) {
    ...
    console.error("No Previous Sunday worship service found");
    return null;
  }
